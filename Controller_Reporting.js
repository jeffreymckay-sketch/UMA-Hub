...
/**
* Fetches all available calendars using the ADVANCED Calendar API.
* Requires the "Calendar" service to be enabled in Apps Script.
*/
function api_getReportCalendars() {
try {
  let calendarList = [];
  let pageToken = null;
  
  // We use a loop to handle pagination if the user has many calendars
  do {
    // Use the Advanced Calendar service (Calendar.CalendarList.list)
    const response = Calendar.CalendarList.list({
      pageToken: pageToken,
      minAccessRole: 'reader' // Only fetch calendars we can read
    });
    
    if (response.items && response.items.length > 0) {
      calendarList = calendarList.concat(response.items);
    }
    
    pageToken = response.nextPageToken;
  } while (pageToken);

  // Map to the object structure the client expects
  const mappedList = calendarList.map(function(cal) {
    return {
      id: cal.id,
      name: cal.summary || "Unnamed Calendar",
      // Advanced API uses 'backgroundColor', Client likely expects 'color'
      color: cal.backgroundColor || "#cccccc" 
    };
  });
  ...
  return {
    success: true,
    data: mappedList,
    message: "Loaded " + mappedList.length + " calendars."
  };

} catch (error) {
  Logger.log("Error in api_getReportCalendars: " + error.message);
  return {
    success: false,
    data: [],
    message: "Error loading calendars (Advanced API): " + error.message
  };
}
}