 ...
 * SECURITY UPDATE: Now fetches strictly from Script Properties to prevent 
 * hardcoding sensitive IDs in the codebase.
 * 
 * @returns {string} The spreadsheet ID.
 * @throws {Error} If the ID is not configured.
 */
function getSpreadsheetId() {
  const settings = getSettings();
  if (!settings.masterSheetId) {
      throw new Error("CRITICAL: Master Spreadsheet ID is not configured. Please run the Setup script or add 'masterSheetId' to Script Properties.");
  }
  return settings.masterSheetId;
}
...
try {
  // Security: Ensure only Admins can map tabs
  requireRole('Admin');
